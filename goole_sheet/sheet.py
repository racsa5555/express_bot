...
def append_products(df):
    ...
    sheet = client.open(title = 'Express-products').sheet1
...
def find_order_by_id(item_id,lang):
    spreadsheet = client.open(title='Express-products')
...
def register_client(data):
    if data.get('ref'):
        spreadsheet = client.open(title='Express-clients')
        sheets = spreadsheet.worksheets()
        sheet = sheets[0]
...
def update_client_by_id(client_id, new_data,ref):
    # if ref:
    #     spreadsheet = client.open('Express-clients')
    #     sheets = spreadsheet.worksheets()
    #     sheet = sheets[0]
    #     data = sheet.get_all_records()
